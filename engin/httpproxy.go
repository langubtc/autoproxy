...
func (proxy *HttpProxyServer)proxyRoundTrip(r *http.Request) (*http.Response, error) {
	...
	total := len(proxy.proxy)
	for i := 0; i < total ; i++ {
		idx := (i + int(proxy.request)) % total

		proxyinstance := proxy.proxy[idx]
		if !IsRemoteAlive(proxyinstance.address) {
			continue
		}

		r.Header.Del("Proxy-Authenticate")
		proxyAddAuth(r, &proxyinstance.auth)

		rsp, err = proxyinstance.RoundTrip(r)
		if err != nil {
			Errorf("proxy round trip fail, %s", err.Error())
			continue
		}else {
			break
		}
