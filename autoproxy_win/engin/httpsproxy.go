...
func HttpsProxyOneHandler(r *http.Request, proxy *proxyServer) (net.Conn,error) {
	...
	logs.Info("connect to %s use remote proxy %s", host, proxy.address)
	...
	if -1 == strings.Index(string(readbuf[:cnt]),"200") {
		logs.Warn("read from remote proxy body :%s ",string(readbuf[:cnt]))
	}
...
func (proxy *HttpProxyServer)HttpsProxyHandler(r *http.Request) (net.Conn,error)  {
	...
	return HttpsProxyOneHandler(r, proxy.proxy)
}
...
func (proxy *HttpProxyServer)HttpsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if e != nil {
		logs.Error("Cannot hijack connection " + e.Error())
	}
	...
	if err != nil {
		logs.Error("address %s: %s", host, err.Error())
	...
	go func() {
		...
		wt := NewWaitGroupTimeout(2, tmout, func() {
			logs.Info("connect from %s to %s timeout",
		...
		logs.Info("connect from %s to %s start", server.RemoteAddr(), client.RemoteAddr())
		...
		logs.Info("connect from %s to %s close", server.RemoteAddr(), client.RemoteAddr())
